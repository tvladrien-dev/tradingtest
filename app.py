...
try:
    ...
    # On importe TradingBotV1Elite qui contient ta stratégie V1 + Probabilités
...
def main():
    ...
    if 'bot' not in st.session_state:
        # Initialisation du nouveau moteur V1 Elite
    ...
    try:
        ...
        # ÉTAPE 2 : Acquisition des flux boursiers (Temps réel 5min)
        with st.spinner(f"Synchronisation de {len(settings.TICKERS_PEA)} actifs..."):
            # On s'assure de récupérer les données avec un intervalle court pour le temps réel
        ...
        for i, (ticker, df) in enumerate(raw_data.items()):
            # Analyse profonde via le moteur V1 Elite (Technique + News + Tendance)
            signal_data = st.session_state.bot.process_ticker(ticker)
            
            if signal_data:
                # Injection de la direction macro
                ...
                if signal_data.get('action') == "ACHAT":
                    ...
                    # On ne notifie que si Probabilité > 70% et nouveau signal
                    should_notify = False
                    if ticker not in st.session_state.notified_tickers:
                        if prob >= 70: should_notify = True
                    else:
                    ...
                    if should_notify:
                        # Utilise le moteur de notification NTFY avec les données complètes (SL ATR, TP, Prob)
        ...
        # --- ÉTAPE 4 : TRIAGE ET RENDU (EXIGENCE : ACHAT EN PRIORITÉ + PROBABILITÉ) ---
        signals_df = pd.DataFrame(all_signals)
        
        if not signals_df.empty:
            # Tri : ACHAT en premier, puis par score de Probabilité décroissant
            signals_df['sort_order'] = signals_df['action'].apply(lambda x: 0 if x == "ACHAT" else (1 if x == "VENTE" else 2))
            signals_df = signals_df.sort_values(by=['sort_order', 'probabilite'], ascending=[True, False])
            
            # Génération du graphique sectoriel avant le rendu
            st.session_state.bot.plot_sectors(all_signals)

        # Rendu de l'Interface Dashboard
        ...
        # ÉTAPE 5 : Gestion du cycle de rafraîchissement (Exigence : 5 min)
        # On utilise le délai configuré (300s par défaut)
        refresh_delay = UPDATE_INTERVAL if 'UPDATE_INTERVAL' in locals() else 300

        dashboard.render_footer()

        # --- AUTO-REFRESH LOGIC (FRANCE TIME) ---
        st.divider()
        placeholder_timer = st.empty()
        
        # Vérification des heures d'ouverture (09:00 - 17:30 pour Euronext)
        if st.session_state.data_loader.check_market_hours():
            for i in range(refresh_delay, 0, -1):
                placeholder_timer.markdown(
                    ...
                    f"🕒 PROCHAIN SCAN DANS {i}s | MODE: TEMPS RÉEL 5MIN"
                    f"</div>", 
            ...
            placeholder_timer.warning("🌙 Marché fermé. Scan en pause jusqu'à 09:00. Mode analyse différée activé.")
